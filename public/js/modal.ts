...
const ICONS: Record<ModalIcon, string> = {
...
class ModalManager {
  ...
  private createModal(options: ModalOptions, resolve: (result: ModalResult) => void): HTMLElement {
    const {
      title,
      content,
      type = 'info',
    ...
    const cancelBtn = modal.querySelector('.modal-btn-cancel') as HTMLButtonElement | null;
  ...
  private getDefaultIcon(type: ModalType): ModalIcon {
    const mapping: Record<ModalType, ModalIcon> = {
      info: 'info',
      success: 'success',
      warning: 'warning',
      error: 'error',
...
export const modal = {
  ...
  success(content: string, title?: string): Promise<ModalResult> {
    return ModalManager.getInstance().show({ type: 'success', content, title });
  },

  warning(content: string, title?: string): Promise<ModalResult> {
    return ModalManager.getInstance().show({ type: 'warning', content, title });
  },

  error(content: string, title?: string): Promise<ModalResult> {
    return ModalManager.getInstance().show({ type: 'error', content, title });
  },

  confirm(content: string, title?: string, options?: Partial<ModalOptions>): Promise<ModalResult> {
