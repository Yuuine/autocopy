...
5.  **原创性原则**：杜绝模板化表达，避免陈词滥调（如"颠覆性"、"黑科技"、"极致体验"等）。每篇文案需有独特的创意角度和表达方式。
6.  **多样性原则**：当需要生成多个版本时，每个版本必须在切入点、叙事结构、情感基调、核心诉求等方面有显著差异，避免重复相似的表达。
...
${versionBriefs.slice(0, count).map((brief, idx) => `【版本${idx + 1}方向】：${brief}`).join('\n')}`;
    ...
    const selectedStrategies = diversityStrategies.slice(0, count);
    const selectedStructures = structureTypes.slice(0, count);
    const selectedEmotions = emotionTones.slice(0, count);

    versionInstructions = `请为每个版本采用完全不同的创作策略，确保内容差异化：

| 版本 | 切入角度 | 叙事结构 | 情感基调 |
|------|----------|----------|----------|
${selectedStrategies.map((s, i) => `| ${i + 1} | ${s} | ${selectedStructures[i]} | ${selectedEmotions[i]} |`).join('\n')}

**差异化要求**：
- 每个版本的开头句必须完全不同，禁止使用相似的表达
- 各版本的核心词汇和句式风格应有明显区别
- 避免在不同版本中重复使用相同的修辞手法或案例
- 确保每个版本都能独立成篇，具有独特的阅读价值`;
  }
